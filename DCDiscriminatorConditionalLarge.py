...
"""
DCGAN discriminator with some custom modifications. Assumes 96x96 (or 128x128) image input.
"""

class DCDiscriminatorConditionalLarge(nn.Module):
    '''
    ...
    def __init__(self, im_chan=3, hidden_dim=64, early_dropout=0.2, 
                        ...
                        class_embed_size = 16, input_dim = 96):
        super(DCDiscriminatorConditionalLarge, self).__init__()
        self.input_image_dim = input_dim
        self.class_embed_size = class_embed_size
        self.late_dropout = late_dropout
        self.hidden_dim = hidden_dim
        
        
        assert input_dim in set([96, 128])
        
        self.final_linear_multiplier = 9 if input_dim == 96 else 25
    ...
    def make_disc_block(self, input_channels, output_channels, kernel_size=4, stride=2, padding = 0, final_layer=False):
        ...
        if not final_layer:
            ...
            return nn.Sequential(
                ...
                nn.Linear(self.hidden_dim * self.final_linear_multiplier, 1),
                nn.Sigmoid()
            )
