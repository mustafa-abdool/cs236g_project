...
class DCGeneratorConditionalLarge(nn.Module):
    '''
    Generator Class
    Values:
    ...
    '''
    def __init__(self, z_dim=16, im_chan=3, hidden_dim=64, output_dim = 128, 
                use_class_embed = False, class_embed_size = 16):
        super(DCGeneratorConditionalLarge, self).__init__()
        self.z_dim = z_dim
        self.use_class_embed = use_class_embed

        assert output_dim in set([128, 96])
        ...
        self.gen = nn.Sequential(
            # if we change kernel_size = 3 here, then we get 96
            self.make_conv_block(self.input_dim, hidden_dim * 16, kernel_size = first_kernel_size, stride = 1, padding = 0),
            self.make_conv_block(hidden_dim * 16, hidden_dim * 8, kernel_size = 4, stride = 2, padding = 1),
